...
case ${RABBITIM_BUILD_TARGERT} in
    android)
        CONFIG_PARA="CC=${RABBITIM_BUILD_CROSS_PREFIX}gcc --disable-shared -enable-static --host=${RABBITIM_BUILD_HOST}"
    ...
    windows_mingw)
		;;
	unix_mingw)
		CONFIG_PARA="${CONFIG_PARA}  CC=${RABBITIM_BUILD_CROSS_PREFIX}gcc --host=${RABBITIM_BUILD_HOST} --enable-sse "
		;;
    *)
		echo "${HELP_STRING}"
		return 2

...
#运行本脚本前,先运行 build_unix_mingw_envsetup.sh 进行环境变量设置,需要先设置下面变量:
#   PREFIX=`pwd`/../unix_mingw  #修改这里为安装前缀
if [ -z "${PREFIX}" ]; then
    echo "build_unix_mingw_envsetup.sh"
    source build_unix_mingw_envsetup.sh
fi
